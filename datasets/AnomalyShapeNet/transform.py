...
class NormalizeCoord(object):
    """Center point cloud and scale into unit sphere."""
...
class CenterShift(object):
    """Shift coordinates so XY center is at origin and optionally align Z bottom."""

    def __init__(self, apply_z: bool = True):
...
class RandomRotate(object):
    """Random rotation around a single axis within a given angle range."""

    def __init__(self, angle=None, center=None, axis: str = "z", always_apply: bool = False, p: float = 0.5):
    ...
    def __call__(self, data_dict):
        ...
        if self.axis == "x":
            ...
            raise NotImplementedError(f"Unsupported rotation axis: {self.axis}")
...
class RandomJitter(object):
    ...
    def __init__(self, sigma: float = 0.005, clip: float = 0.02, p: float = 0.5):
        self.sigma = sigma
        self.clip = clip
        self.p = p
    ...
    def __call__(self, data_dict):
        ...
        if "coord" in data_dict:
            jitter = np.clip(
                self.sigma * np.random.randn(*data_dict["coord"].shape), -self.clip, self.clip
            )
...
class RandomScale(object):
    """Random isotropic scaling within [scale_low, scale_high]."""

    def __init__(self, scale_low: float = 0.9, scale_high: float = 1.1, p: float = 0.5):
...
class SphereCropMask(object):
    """Randomly sample multiple spherical regions and assign mask ids.

    Points not assigned to any region receive mask id > part_num.
    """

    def __init__(self, part_num: int = 64):
